		...
		private void ValidateRecordTypes()
		{
			...
			if (DataStorage1.RecordType != DataStorage2.RecordType)
			{
				...
				if (mConvert1to2 == null)
					throw new BadUsageException("You must to define a method in the class " + DataStorage1.RecordType.Name 
                                              + " with the attribute [TransfortToRecord(typeof(" + DataStorage2.RecordType.Name + "))]");

				mConvert2to1 = GetTransformMethod(DataStorage2.RecordType, DataStorage1.RecordType);
				if (mConvert2to1 == null)
					throw new BadUsageException("You must to define a method in the class " + DataStorage2.RecordType.Name 
                                               + " with the attribute [TransfortToRecord(typeof(" + DataStorage1.RecordType.Name + "))]");
			}
		}
